...
N_inst = sys.argv[3]
if int(N_inst)<10:
    inst_name="Project\\Instances\\inst0"+N_inst+".dat"
    simple = True
else:
    inst_name="Project\\Instances\\inst"+N_inst+".dat"
    simple = False
...
if method=="CP":
    if solver not in ["gecode", "chuffed"]:
        ...
        dict=Solve_CP(m, n, l, s, Dnp, UB, LB, LLB, simple, solver)
        print(dict)
elif method=="SAT":
...
file_name = 'Project\\res\\'+method+'\\'+N_inst+'.json'
path = Path(file_name)
if path.is_file():
    f = open(file_name, 'r')
